...
//* absent on the detail and seller-dashboard responses.
 String? get sellerName;//* Seller's display name + profile photo (storage path), joined from
...
//* the real seller — never a mock. Null when the seller has no photo.
 String? get sellerAvatarUrl; double? get distanceKm; bool? get inRange; double? get rating; int? get reviewCount;//* Seller pickup coordinates — buyer-feed only, used to place real
...
@JsonKey(includeFromJson: false, includeToJson: false)
@override
int get hashCode => Object.hashAll([runtimeType,id,sellerId,name,description,const DeepCollectionEquality().hash(imageUrls),priceCents,originalPriceCents,discountPercent,portionsLeft,const DeepCollectionEquality().hash(cuisineTypes),const DeepCollectionEquality().hash(dishTypes),const DeepCollectionEquality().hash(dietaryTags),const DeepCollectionEquality().hash(allergens),otherAllergens,isAvailable,isVeg,menuCategory,category,fulfillment,prepMinutes,expiresAt,createdAt,updatedAt,const DeepCollectionEquality().hash(extras),sellerName,sellerAvatarUrl,distanceKm,inRange,rating,reviewCount,lat,lng]);

@override
String toString() {
  return 'Listing(id: $id, sellerId: $sellerId, name: $name, description: $description, imageUrls: $imageUrls, priceCents: $priceCents, originalPriceCents: $originalPriceCents, discountPercent: $discountPercent, portionsLeft: $portionsLeft, cuisineTypes: $cuisineTypes, dishTypes: $dishTypes, dietaryTags: $dietaryTags, allergens: $allergens, otherAllergens: $otherAllergens, isAvailable: $isAvailable, isVeg: $isVeg, menuCategory: $menuCategory, category: $category, fulfillment: $fulfillment, prepMinutes: $prepMinutes, expiresAt: $expiresAt, createdAt: $createdAt, updatedAt: $updatedAt, extras: $extras, sellerName: $sellerName, sellerAvatarUrl: $sellerAvatarUrl, distanceKm: $distanceKm, inRange: $inRange, rating: $rating, reviewCount: $reviewCount, lat: $lat, lng: $lng)';
}
...
switch (_that) {
case _Listing() when $default != null:
return $default(_that.id,_that.sellerId,_that.name,_that.description,_that.imageUrls,_that.priceCents,_that.originalPriceCents,_that.discountPercent,_that.portionsLeft,_that.cuisineTypes,_that.dishTypes,_that.dietaryTags,_that.allergens,_that.otherAllergens,_that.isAvailable,_that.isVeg,_that.menuCategory,_that.category,_that.fulfillment,_that.prepMinutes,_that.expiresAt,_that.createdAt,_that.updatedAt,_that.extras,_that.sellerName,_that.sellerAvatarUrl,_that.distanceKm,_that.inRange,_that.rating,_that.reviewCount,_that.lat,_that.lng);case _:
  return orElse();
